...
CAS_INSTALLER_FILE=/tmp/OCcas11.3.0-Linux64.bin
MDEX_INSTALLER_FILE=/tmp/OCmdex11.3.0-Linux64_1186050.bin
PLATFORM_INSTALLER_FILE=/tmp/OCplatformservices11.3.0-Linux64.bin
...
if [ "`ls -A /opt/endeca`" = "" ]; then
    ...
    echo "-fileOverwrite_/opt/endeca/MDEX/11.3.0/Uninstall/Uninstall_Oracle Commerce MDEX Engine 11.3.0 x64 Edition.lax=Yes" >> /tmp/mdex_silent.txt
    $MDEX_INSTALLER_FILE -i silent -f /tmp/mdex_silent.txt
    source /opt/endeca/MDEX/11.3.0/mdex_setup_sh.ini
    ...
    echo "EAC_MDEX_ROOT=/opt/endeca/MDEX/11.3.0" >> /tmp/platform_silent.txt
    echo "-fileOverwrite_/opt/endeca/PlatformServices/11.3.0/Uninstall/Uninstall_Oracle Commerce Guided Search Platform Services 11.3.0.lax=Yes" >> /tmp/platform_silent.txt
    ...
    echo "ENDECA_TOOLS_ROOT=/opt/endeca/ToolsAndFrameworks/11.3.0" >> /tmp/cas_silent.txt
    echo "ENDECA_TOOLS_CONF=/opt/endeca/ToolsAndFrameworks/11.3.0/server/workspace" >> /tmp/cas_silent.txt
    echo "-fileOverwrite_/opt/endeca/CAS/11.3.0/uninstall/CAS11.3.0_uninstall/Uninstall Oracle Commerce Content Acquisition System 11.3.0.lax=Yes" >> /tmp/cas_silent.txt
    echo "-fileOverwrite_/opt/endeca/ToolsAndFrameworks/11.3.0/server/workspace/conf/Standalone/localhost/casconsole.xml=Yes" >> /tmp/cas_silent.txt
    echo "-fileOverwrite_/opt/endeca/ToolsAndFrameworks/11.3.0/server/webapps/casconsole-11.3.0-ssl.war=Yes" >> /tmp/cas_silent.txt
    echo "-fileOverwrite_/opt/endeca/ToolsAndFrameworks/11.3.0/server/webapps/casconsole-11.3.0.war=Yes" >> /tmp/cas_silent.txt
    echo "-fileOverwrite_/opt/endeca/ToolsAndFrameworks/11.3.0/server/workspace/conf/casconsole.properties=Yes" >> /tmp/cas_silent.txt
	...
	sed -i '/ENDECA_TOOLS_CONF=/d' /opt/endeca/ToolsAndFrameworks/11.3.0/server/bin/workbench.sh
else
...
ENDECA_ROOT=/opt/endeca/PlatformServices/11.3.0 ; export ENDECA_ROOT
ENDECA_CONF=/opt/endeca/PlatformServices/workspace ; export ENDECA_CONF
ENDECA_TOOLS_ROOT=/opt/endeca/ToolsAndFrameworks/11.3.0 ; export ENDECA_TOOLS_ROOT
...
su developer -c "nohup /opt/endeca/CAS/11.3.0/bin/cas-service.sh < /dev/null > /opt/endeca/CAS/workspace/logs/cas-service-wrapper.log &"

shutdown(){
    su developer -c "$ENDECA_ROOT/tools/server/bin/shutdown.sh"
    su developer -c "$ENDECA_TOOLS_ROOT/server/bin/shutdown.sh"
    su developer -c "/opt/endeca/CAS/11.3.0/bin/cas-service-shutdown.sh"
	END=1
}
